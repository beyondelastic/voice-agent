"""Health-insurance tools for the Foundry agent example.

Sample data lives in the repo-level data/data.json (shared with the native
...
definitions (TOOL_SCHEMAS), and a run_tool() dispatcher. create_agent.py
converts TOOL_SCHEMAS into azure.ai.projects.models.FunctionToolParam objects
(attached to the agent), and voice_agent.py runs the calls via run_tool().
"""
